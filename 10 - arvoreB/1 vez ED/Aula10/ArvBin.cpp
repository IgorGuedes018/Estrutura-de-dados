...
int ArvBin::contaNos()
{
    ...
    int cont =0;
    return auxConta(raiz, cont);


}
int ArvBin::auxConta(NoArv *p , int cont )
{

    if(p != NULL)
    {
        
        cont= 1 + (auxConta(p->getEsq(), cont)) + (auxConta(p->getDir(), cont)) ;
...
int ArvBin::auxBusca(NoARv *p, int cont)
{
    if(p != NULL)
    {
        if(p->getEsq == NULL && p->getDir == NULL)
            cont++;   
        auxBusca(p->getEsq, int cont);  
        auxBusca(p->getDir, int cont);
    }
    return cont;
}
